...
//    var linkCreate: RouterLink = RouterLink("Создать агента", MainView::class.java)

    init {
        val agents: List<Agent> = service.getAll()
        if (agents.isNotEmpty()) {
            ...
            grid.addColumn { it.lastResponseTime?.howLongAgoItWasBeauty() }.setHeader("Последний отклик").isAutoWidth =
                true
...
//        sd
        ...
        layout.add(grid)
        addToNavbar(H3("Список существующих агентов"))
        content = layout
    }
